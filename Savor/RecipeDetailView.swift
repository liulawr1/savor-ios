...
struct RecipeDetailView: View {
    ...
    var current: Recipe { store.current(recipe) }
    var isSaved: Bool { store.saved.contains { $0.id == recipe.id } }
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 23) {
                ...
                HStack { Pill(text: recipe.isSample ? "Sample recipe" : "AI suggestion", symbol: "sparkles"); Spacer(); Text("\(recipe.minutes) MIN · SERVES \(recipe.servings)").font(.system(size: 10, weight: .bold, design: .monospaced)).foregroundStyle(Palette.muted) }
                Text(recipe.title).font(.system(size: 37, design: .serif)).tracking(-0.7)
                Text(recipe.description).font(.subheadline).foregroundStyle(Palette.muted).lineSpacing(4)
                HStack(alignment: .top, spacing: 12) { Image(systemName: "sun.max").foregroundStyle(Palette.orange); Text(recipe.why).font(.subheadline).lineSpacing(3) }.savorCard(Color(red: 0.98, green: 0.93, blue: 0.84))
                HStack { Text("The ingredients").font(.system(size: 27, design: .serif)); Spacer(); Text("\(recipe.ingredients.count) items").font(.caption).foregroundStyle(Palette.muted) }
                VStack(spacing: 15) {
                ...
                HStack { Text("Let’s make it").font(.system(size: 27, design: .serif)); Spacer(); Text("\(current.completedSteps.count)/\(recipe.steps.count)").font(.caption.monospacedDigit()).foregroundStyle(Palette.muted) }
                ProgressView(value: current.progress).tint(Palette.green)
                VStack(spacing: 12) {
                    ForEach(Array(recipe.steps.enumerated()), id: \.offset) { index, step in
                        Button { store.toggleStep(index, recipe: recipe) } label: {
                ...
                else { ActionButton(title: "I made this", symbol: "checkmark", disabled: current.completedSteps.count != recipe.steps.count) { finishing = true }.accessibilityIdentifier("finishCooking") }
        ...
        .toolbar { ToolbarItemGroup(placement: .topBarTrailing) { ShareLink(item: recipe.shareText) { Image(systemName: "square.and.arrow.up") }; Button { store.save(recipe) } label: { Image(systemName: isSaved ? "bookmark.fill" : "bookmark") }.disabled(isSaved).accessibilityLabel(isSaved ? "Recipe saved" : "Save recipe").accessibilityIdentifier("saveRecipe") } }
